...
export default function ViewStudentPage() {
  ...
  const fetchData = async () => {
    return axios
      .get(`${process.env.REACT_APP_HOG_API}/api/Student/Get/${id}`, config)
      .then((res) => {
        const data = res.data.data;
        setStudent(data);
        const pathReference = ref(storage, `users/students/${data.firebaseId}/Avatar/${data.profilePicture}`);
        getDownloadURL(pathReference).then((url) => setAvatarURL(url));
          ...
          action={
            <>
              <Stack direction="row" spacing={2}>
                ...
                <Button
                  component={Link}
                  to={`/account/student-management/student/${id}/edit`}
                  size="large"
                  variant="contained"
                >
                  Edit Student
