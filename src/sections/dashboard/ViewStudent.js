...
import { Box, Card, Chip, Grid, Stack, Typography, TextField, Button } from '@mui/material';
...
import ActivateAccountDialog from './oa-edit-account/ActivateAccountDialog';
import DisableAccountDialog from './oa-edit-account/DisableAccountDialog';
...
export function StudentInfo({ student }) {
  ...
  const [openDisableAccountDialog, setOpenDisableAccountDialog] = useState(false);
      ...
      <Card sx={{ p: 3 }}>
        <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2 }}>
          ...
          <Button
            ...
            onClick={() => (student.isActive ? setOpenDisableAccountDialog(true) : setOpenActivateAccountDialog(true))}
            sx={{
              ':hover': {
                backgroundColor: student.isActive ? 'error.lighter' : 'primary.lighter',
                color: student.isActive ? 'error.main' : 'primary.main',
                borderColor: student.isActive ? 'error.main' : 'primary.main',
              },
            }}
          >
            {student.isActive && !hover && 'Active'}
            {!student.isActive && !hover && 'Inactive'}
            {student.isActive && hover && 'Disable'}
            {!student.isActive && hover && 'Activate'}
          </Button>
        </Stack>
        ...
        <DisableAccountDialog
          ...
          accountRole="Student"
          accountName={student.fName}
          open={openDisableAccountDialog}
          onClose={() => setOpenDisableAccountDialog(false)}
        />
      ...
      <Grid container direction="row" spacing={2}>
        <Grid item xs={12} md={12}>
          <TextField
            fullWidth
            disabled
            label="Health information that needs to be caution"
            defaultValue={student.healthInfo}
          />
        </Grid>
        <Grid item xs={12} md={12}>
          <TextField
            fullWidth
            disabled
            label="How did student know about House of Griffin?"
            defaultValue={student.hogInfo}
          />
        </Grid>
      </Grid>
    </Card>
      ...
      <Stack direction="row">
        {filesURL.map((file) => {
          return (
            ...
            >
              <FileThumbnail tooltip imageView file={file} onDownload={() => window.open(`${file.preview}`)} />
            </Stack>
          );
        })}
      </Stack>
