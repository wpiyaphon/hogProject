...
export default function ActivateAccountDialog({ accountId, accountRole, accountName, open, onClose }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      if (accountRole === 'Teacher') {
        await axios.get(`${HOG_API}/api/Teacher/Enable/${accountId}`).catch((error) => {
          throw error;
        });
        ...
        await axios.get(`${HOG_API}/api/Student/Enable/${accountId}`).catch((error) => {
          throw error;
        });
        enqueueSnackbar('Activate the account successfully', { variant: 'success' });
        navigate('/account/student-management/student');
      } else {
        await axios.get(`${HOG_API}/api/Staff/Enable/${accountId}`).catch((error) => {
          throw error;
        });
        enqueueSnackbar('Activate the account successfully', { variant: 'success' });
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={onClose}>
      ...
      <DialogContent>{`Once activated, ${accountName}'s account will be accessible.`}</DialogContent>
      <DialogActions>
        <Button variant="outlined" color="inherit" onClick={onClose}>
          Cancel
        </Button>
        <LoadingButton type="submit" variant="contained" color="success" loading={isSubmitting} onClick={handleDelete}>
          Activate
        </LoadingButton>
      </DialogActions>
    </Dialog>
