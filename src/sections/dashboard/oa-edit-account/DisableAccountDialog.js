...
export default function DisableAccountDialog({ accountId, accountRole, accountName, open, onClose }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (accountRole === 'Teacher') {
        await axios.delete(`${HOG_API}/api/Teacher/Disable/${accountId}`).catch((error) => {
          throw error;
        });
        ...
        await axios.delete(`${HOG_API}/api/Student/Disable/${accountId}`).catch((error) => {
          throw error;
        });
        enqueueSnackbar('Disable the account successfully', { variant: 'success' });
        navigate('/account/student-management/student');
      } else {
        await axios.delete(`${HOG_API}/api/Staff/Disable/${accountId}`).catch((error) => {
          throw error;
        });
        enqueueSnackbar('Disable the account successfully', { variant: 'success' });
  ...
  return (
    <Dialog fullWidth maxWidth="sm" open={open} onClose={onClose}>
      ...
      <DialogContent>{`Once disabled, ${accountName}'s account will no longer be accessible.`}</DialogContent>
      <DialogActions>
        <Button variant="outlined" color="inherit" onClick={onClose}>
          Cancel
        </Button>
        <LoadingButton type="submit" variant="contained" color="error" loading={isSubmitting} onClick={handleDelete}>
          Disable
        </LoadingButton>
      </DialogActions>
    </Dialog>
